...
- (void)viewDidLoad {
    ...
    if(self.navigationController==nil){ //判断当前是否有导航，如果没有导航则给创建一个关闭退出当前界面的一个按钮
        
    UIButton* btn = [[UIButton alloc] initWithFrame:CGRectMake(view.frame.size.width-60, 32, 50, 50)];
        
        [btn setImage: [UIImage imageNamed:[@"CustomSettings.bundle" stringByAppendingPathComponent:@"PanoramaClose.png"]] forState:UIControlStateNormal];
        [btn addTarget:self action:@selector(CloseClickEvent:) forControlEvents:UIControlEventTouchUpInside];
    [view addSubview:btn];
...
-(void)CloseClickEvent:(UIButton *)btn{
