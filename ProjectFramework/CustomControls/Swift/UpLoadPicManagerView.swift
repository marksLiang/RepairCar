...
class UpLoadPicManagerView: UIView, UICollectionViewDelegate, UICollectionViewDataSource,TZImagePickerControllerDelegate {
    ...
    /// 自增高
   fileprivate var Sincethehigher=false
    ///竖屏显示的图片数 （暂时未做横屏手机item  间距直接也存在点差异 ipad 没事 iPhone需要修改
    ///默认是4张   建议最多一行设置不要超过5张
   fileprivate var Showrowsitem:CGFloat=4
    /// 最多能选择的图片总数  默认10张
   fileprivate var SelectedImgMaxCount:Int  = 10
    ...
    init(frame: CGRect,delegate:UIViewController,ShowRowsItem:CGFloat,SelectedImgMaxCount:Int,ImgHeight:CGFloat=0,Sincethehigher:Bool=false,Callback_SelectedValue:@escaping CallbackSelectedValue) {
        super.init(frame: frame)
        self.myCallbackValue=Callback_SelectedValue
        self.delegate=delegate
        self.Showrowsitem=ShowRowsItem
        self.SelectedImgMaxCount=SelectedImgMaxCount
        self.GetImgHeight=ImgHeight
        self.Sincethehigher=Sincethehigher
        load()
    }
    ...
    /// 添加网络图片  ( 第一次配置的时候添加 )
    ///
    /// - parameter imgUrl: imgurl数组
    func SetImageUrl(_ imgUrl:[String]){
            ...
            if(data != nil ){
               let img = UIImage(data: data as! Data)
                if(img != nil ) {
                    ...
                    self.ImgItem.append(UIImage(named: placeholderImage)!)
                }
            }else{
               self.ImgItem.append(UIImage(named: placeholderImage)!)
            }
