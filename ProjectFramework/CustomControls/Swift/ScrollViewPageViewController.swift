...
class ScrollViewPageViewController: UIViewController, UIScrollViewDelegate {
    ...
    func initLoad(){
        ...
        for i in ImageList {
            ...
            if(imageView.image == nil ) {   //判断图片是否为空，为空则认为是网络图片
                imageView.sd_setImage(with:URL(string:HttpsUrlImage+i), placeholderImage:  UIImage(named: placeholderImage) ,options:  SDWebImageOptions.retryFailed) { (UIImage, NSError, SDImageCacheType, NSURL) -> Void in
                    if(UIImage != nil){
