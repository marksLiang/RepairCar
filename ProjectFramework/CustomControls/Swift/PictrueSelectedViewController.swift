...
class PictrueSelectedViewController:UIViewController, UICollectionViewDelegate, UICollectionViewDataSource    {
    ...
    private var assets = [ALAsset]()//保存照片集合
    private  var CheckItem=NSHashTable<AnyObject>()  //选中的(图片)HashTable集合
    private  var bartranslucent=false   //监测translucent=false的时候记录
