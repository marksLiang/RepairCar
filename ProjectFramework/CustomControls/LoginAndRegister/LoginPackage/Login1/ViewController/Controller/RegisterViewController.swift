...
class RegisterViewController: UITableViewController,UITextFieldDelegate {
    ...
    //获取验证码回调事件
    typealias CallbackVerificationCodeValue=(_ phone:String)->Void
    ...
    override func viewDidLoad() {
        ...
        let tapGRs = UITapGestureRecognizer(target: self, action:#selector(RegisterViewController.tapHandlermoreKey(_:)))    //点击手势
    ...
    @IBAction func GetVerificationCode(_ sender: AnyObject) {
        ...
        if(!Validate.phoneNum(phone.text!).isRight){
            CommonFunction.HUD("非法手机号码", type: .error)
            return
        }
        if(VerificationCodeCallbackValue != nil ){
            hidekeyboard()
            VerificationCodeCallbackValue!(self.phone.text!)
        }
       //启动计时器
    ...
    func VerificationCodeTime(){
        ...
        if(IntervalInitialize>=timerMaxInterval){
            ...
            VerifiacationCodeBtn.backgroundColor=CommonFunction.SystemColor() //设置颜色
